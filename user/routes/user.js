...
router.get('/getSavedBills/:email', function(req, res, next) {
    
    console.log("RWQ******", req.params.email);
    let email = req.params.email;
    var query = 'SELECT savedbills from user WHERE email = ?';
    execute(query, [email], (err, result) => {
        if(err) {
            console.log("fail");
            response.statusCode = 500;
            ...
            console.log("successfullbills");
            console.log(result);
            response.statusCode = 200;
            response.message = "successfully added to watlist";
            response.savedbills = result.rows[0].savedbills;
            res.send(response);
        }
    });
});
